// ==========================================
// 🔐 मास्टर ॲक्टिव्हेशन की (Obfuscated)
// ==========================================
...
// 🎤 Voice Typing (Language Fixed to mr-IN as requested)
function startDictation(targetId) {
    try {
        var SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SpeechRecognition) { alert("⚠️ 'व्हॉइस टायपिंग' सपोर्ट नाही."); return; }
        var recognition = new SpeechRecognition(); recognition.continuous = false; recognition.interimResults = false; 
        recognition.lang = "mr-IN"; // Changed back to Marathi
        ...
        recognition.onerror = function(e) { recognition.stop(); if(btn) { btn.style.backgroundColor = "#e8f8f5"; btn.style.color = "black"; } console.warn("Mic error:", e); };
...
// 🚨 Security Fix: SHA-256 Hashing for PINs
...
function getDB(table) { try { var data = JSON.parse(localStorage.getItem(table)); return Array.isArray(data) ? data : []; } catch(e) { console.error("Data Corrupted:", e); return []; } }
...
// 💾 Storage Limit Fix (Warning at 3MB)
function checkStorageLimit() { 
    ...
    if(mb > 3.0) { alert(`⚠️ धोक्याचा इशारा: ॲपची मेमरी ${mb}MB भरली आहे (मर्यादा 5MB). कृपया डेटा बॅकअप घ्या!`); } 
}
...
// २. 🚫 अँटी-पायरसी & ३. लॉगिन (Async)
async function checkActivation() { 
    var authHash = localStorage.getItem('appAuthHash'); 
    var expectedHash = await hashPin('activated_true');
    if(authHash !== expectedHash) { document.getElementById('activationScreen').style.display = "flex"; document.getElementById('lockScreen').style.display = "none"; return false; } 
    else { document.getElementById('activationScreen').style.display = "none"; return true; } 
}
async function activateApp() { 
    var key = document.getElementById('activationKey').value.trim(); 
    if(key === _0x1a2b[0]) { localStorage.setItem('appAuthHash', await hashPin('activated_true')); alert("✅ ॲप ॲक्टिव्हेट झाले!"); document.getElementById('activationScreen').style.display = "none"; document.getElementById('lockScreen').style.display = "flex"; } 
    else { alert("❌ चुकीची ॲक्टिव्हेशन की!"); } 
}

var currentUserRole = 'owner'; 
async function checkPin() {
    var defaultOwnerHash = await hashPin("1234"); var defaultStaffHash = await hashPin("0000");
    var ownerHash = localStorage.getItem('shopPinHash') || defaultOwnerHash; 
    var staffHash = localStorage.getItem('staffPinHash') || defaultStaffHash; 
    var enteredPin = document.getElementById('loginPin').value.trim();
...
async function saveShopSettings() { 
    localStorage.setItem('shopName', cleanText(document.getElementById('shopNameInput').value)); localStorage.setItem('shopAddress', cleanText(document.getElementById('shopAddressInput').value)); localStorage.setItem('shopPhone', cleanText(document.getElementById('shopPhoneInput').value)); localStorage.setItem('shopGstin', cleanText(document.getElementById('shopGstinInput').value).toUpperCase()); 
    var logoData = document.getElementById('shopLogoInput').dataset.base64; if(logoData) localStorage.setItem('shopLogo', logoData); 
    var oPin = document.getElementById('shopPinInput').value.trim(); if(oPin) localStorage.setItem('shopPinHash', await hashPin(oPin)); 
    var sPin = document.getElementById('staffPinInput').value.trim(); if(sPin) localStorage.setItem('staffPinHash', await hashPin(sPin)); 
    showMessage("⚙️ सेटिंग सेव्ह झाले!", "green"); loadShopSettings(); 
}
...
function saveExpense() { 
    ...
    try {
        isProcessing = true;
        var editId = document.getElementById('editExpId').value; var desc = cleanText(document.getElementById('expDesc').value); var amount = parseFloat(document.getElementById('expAmount').value) || 0; var mode = document.getElementById('expPaymentMode').value; var utr = cleanText(document.getElementById('expUtrNo').value);
...
function saveStaff() { 
    ...
    try {
        ...
        else { if(staff.find(s => s.name.trim().toLowerCase() === name.trim().toLowerCase())) { alert("⚠️ हा कर्मचारी आधीपासूनच जोडलेला आहे!"); return; } staff.push({staff_id: new Date().getTime().toString(), name: name.trim(), salary, advance_balance: advance, payment_history: []}); setDB('Staff', staff); }
