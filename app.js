...
// १. अचूक तारीख आणि उपयुक्त फंक्शन्स
...
// ==========================================
// 🚨 NEW: Centralized XSS Validation Layer (Firewall)
// ==========================================
...
// Upgrade Legacy Hashes to SHA-256
...
// 💾 Secure Database Setter (Data Guard with Firewall)
function setDB(table, rawData) { 
    try { 
        var safeData = sanitizeData(rawData); // 🔥 Firewall
...
// ==========================================
// 💸 दैनंदिन खर्च (Daily Expenses) - CA Categories
// ==========================================
...
// ==========================================
// 💊 औषधे व्यवस्थापन (GST & Rack Update)
// ==========================================
...
function saveMedicine() {
    ...
    try {
        ...
        var gstRate = parseInt(document.getElementById('medGstRate').value) || 0; 
...
function displayTodayBills() {
    ...
    todaySales.forEach(sale => { var isCred = sale.is_credit ? "<span style='color:red;'>(उधारी)</span>" : "<span style='color:green;'>(रोख)</span>"; var drText = sale.doctor_name ? `<br><small style="color:#8e24aa;">Dr. ${sale.doctor_name}</small>` : ""; listBody.innerHTML += `<tr><td><b>${sale.customer_name}</b> ${isCred}${drText}<br><small style="color:gray;">${sale.bill_id}</small></td><td style='color: #2e7d32; font-weight: bold;'>₹${(sale.total_amount || 0).toFixed(2)}</td><td><button onclick="deleteBill('${sale.bill_id}')" class="action-btn delete-btn">रद्द</button></td></tr>`; });
}
...
function displayCustomers() { 
    ...
    custs.forEach(c => { 
        var balText = c.ledger_balance <= 0 ? `₹${(Math.abs(c.ledger_balance) || 0).toFixed(2)}` : `₹${(c.ledger_balance || 0).toFixed(2)}`; var balColor = c.ledger_balance <= 0 ? "green" : "red"; var histHTML = ""; 
